...
class Podman(Generic):
    ...
    def serve(self, log=False):
        def launch_server(return_vals):
            args = ["podman", "run", "--rm", "-d", "-p", "127.0.0.1:3000:80", "alpine-http-server"]
    ...
    def ping(self, log=False):
        args = ["podman", "run", "--rm", "alpine-network"]
    ...
    def launch_read(self, log=False):
        args = ["podman", "run", "--rm", "alpine-mondial-read"]
        output = subprocess.run(args, stdout=subprocess.PIPE)
        if log:
            print(output)

    def launch_write(self, log=False):
        args = ["podman", "run", "--rm", "alpine-mondial-write"]
        output = subprocess.run(args, stdout=subprocess.PIPE)
        if log:
            print(output)
