...
def main(plots_f, sols):
    # # #
    # Docker alpine crun
    tag = 'Podman alpine crun rootless'
    s = ["podman-alpine-crun-btrfs-rootless", "podman-alpine-crun-overlay-rootless", "podman-alpine-crun-vfs-rootless"]
    ...
    # # #
    # Docker centos crun
    tag = 'Podman centos crun rootless'
    s = ["podman-centos-crun-btrfs-rootless", "podman-centos-crun-overlay-rootless", "podman-centos-crun-vfs-rootless"]
    t = ["Hello World", "Http server", "Network", "Ping"]
    benchmark_solutions = {key: value for (key, value) in sols.items() if key in s}
    plots.plot_benchmarks(results_operations.group_benchmarks(benchmark_solutions, t), tag, plots_f)
    plots.plot_ios(sols, s, tag, plots_f)
